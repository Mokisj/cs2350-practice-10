            ...
            <div class="row g-0">
                ...
                <div class="col-md-8">
                    <div class="card-body">
                        ...
                        <p class="card-text">
                        <button class="btn btn-danger to-delete">Delete</button>                        </p>
                    </div>
                </div>
            </div>
        </div>
...
document.querySelectorAll('.to-delete').forEach(btn()){
    btn.onclick = function(event){
        if(confirm("Are you sure you want to delete this card")){
            cards.slice(event.target.closest('.col').dataset.ndx, 1)
            localStorage.setItem("cards", JSON.striungify(cards))
            displayCards()
        }
    }
}
